...
# find its size, and pick a random element from [0, size - 1].
...
# On the ith iteration of our loop to pick a random element,
# let’s assume we already picked an element uniformly from [0, i - 1].
# In order to maintain the loop invariant,
# we would need to pick the ith element as the new random element at 1 / (i + 1) chance.
